...
export default function ProjectCard() {
  ...
  return (
    <div className="flex w-full max-w-full flex-col gap-10 overflow-x-clip">
      {projectsData.map((project, index) => {
        ...
        return (
          <motion.article
            key={project.title}
            className={`group flex w-full max-w-full flex-col overflow-hidden rounded-[2.2rem] border border-slate-200 bg-slate-50 p-5 shadow-lg transition hover:border-cyan-800 hover:shadow-2xl hover:shadow-cyan-900/10 lg:min-h-[430px] lg:p-8 dark:border-slate-800 dark:bg-slate-900/60 dark:hover:border-cyan-400 ${
          ...
          >
            <div className="relative h-[230px] w-full max-w-full overflow-hidden rounded-[1.7rem] lg:mt-0 lg:h-auto lg:flex-1">
              <Image
                src={project.image}
                alt={project.title}
                fill
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-center p-3 pt-6 lg:p-6">
              <h3 className="break-words text-3xl font-semibold text-slate-950 sm:text-4xl dark:text-white">
                {project.title}
              </h3>
